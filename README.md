# php pdo_db class
pdo class connect mysql in php
Initialize
------------
```php
<?php
...
define('DBUser', 'root');
define('DBPassword', '');
require(dirname(__FILE__)."/conn/PDO.class.php");
...
```php
<?php
$DB->getRow("SELECT * FROM fruit WHERE name=? and color=?",array('apple','red'));
?>
```
...
```php
<?php
$DB->getOne("SELECT color FROM fruit WHERE name=? ",array('watermelon'));
?>
```
